...
import { CFR_COLUMNS } from '@/lib/export-columns'
...
export function CfrSpokeShell({ spoke }: { spoke: CorpusSpoke }) {
  ...
  // Usage log (ragtime-usage-log-feedback): a fresh id + the question are
  // captured when a result lands so the annotation bar can log the trace.
  ...
  async function runAmaExecute(plan: CfrAmaPlan, question: string) {
    if (!auth.auth) return
    appendLog({
      label: 'Step 2/3.',
  ...
  function downloadFilterCsv() {
    ...
    downloadCsv('ragtime-cfr-results.csv', {
      title: 'RAGtime — CFR export',
      meta: [
