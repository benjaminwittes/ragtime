...
- Typography: serif (EB Garamond) for headings, sans (Lato) for body. Both **self-hosted** — `src/fonts.css` and `src/assets/fonts/`, regenerated by `node scripts/fetch-fonts.mjs`. Use `font-serif` / `font-sans` Tailwind classes.
