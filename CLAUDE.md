...
- **Claude Cowork** runs on Ben's primary computer. Owns iteration on **`index.html` only**, and only UI / copy / feature-tweak changes (visual, layout, text, small interaction tweaks). Cowork commits and pushes `index.html` directly to GitHub. Cowork does **not** edit billing, auth, quota logic, the Worker, the data-acquisition pipeline, or anything backend. Cowork also operates the editorial-side scheduled tasks (status checks, queue-depth monitoring) that read from Supabase but doesn't directly run the pipeline daemons. **(Confirmed 2026-05-21: pipeline *deploys/ops* moved to Claude Code on Mac2; pipeline *monitoring* deliberately STAYS on Cowork — a monitor must run independently of the machine it watches, or it goes dark exactly when the machine it monitors does.)**
- **Claude Code** runs on the residential Mac that *also* hosts the running data-acquisition pipeline ("Mac2"). Owns **all source code**: the Cloudflare Worker (now in `worker/` with its `wrangler.toml` + GitHub Actions deploy), infrastructure config, tests, documentation, the eventual modular split of the frontend, and any productization-driven fixes that need to touch `index.html`. The **data-acquisition pipeline source lives in a separate PRIVATE repo, `benjaminwittes/ragtime-pipeline`** (api-0 daemon, litigation-sync, backfill, classify.py) — see the pipeline-repo decision note below for why it's separate and private. Because it sits on the same machine as the running pipeline, Claude Code also drives the pipeline's deploys (`git pull && ./install.sh` from the private repo), launchd start/stop, log inspection, and install-script runs.
- **If Claude Code edits `index.html`**, it is flagged to Ben so Cowork pulls before its next prototype-iteration session.
- **Branching (as of 2026-05-21):** Claude Code works on feature branches and opens PRs; Cowork continues to commit and push `index.html` directly to `main`. Cowork should `git pull` `main` before each session to pick up merged Code changes.
...
- **Branching model:** Claude Code works on feature branches + PRs (as of 2026-05-21); Cowork still pushes `index.html` directly to `main`. Branch protection not yet enforced.
